...
# one variable plots
# density of price
ggplot(data = AB_NYC) + geom_density(mapping = aes(x=???))
...
ggplot(data = AB_NYC) +
  geom_density(mapping = aes(x = price, color = ???)) + 
  xlim(0,500)
...
# combining the 2 above plots:
# boxplot to see price by neighbourhood_group and room_type (via color or fill)
base + 
  geom_boxplot(mapping = aes(x = neighbourhood_group, y = price, fill = ???)) + 
  ylim(0, 500)
...
manhattan_data = 
  AB_NYC %>% 
  filter(neighbourhood_group == "???", room_type == "???")

# scatterplot of number_of_reviews and price
# looking only at Private Rooms in Manhattan
# add availability_365 as a color
ggplot(data = ???) +
  geom_point(mapping = aes(x = price, y = number_of_reviews, color = ???))

# adding a column with mutate()
# let's calculate the length of the listing's title (name) 
# on the manhattan_data
manhattan_data = 
  ??? %>% 
  mutate(title_length = str_length(???))
...
# things to calculate for each calculated_host_listings_count:
#   - number of listings
#   - mean price 
...
summary = 
  ...
  summarise(
    num_listings = n(),
    mean_price = mean(???),
    mean_num_reviews = mean(???),
    mean_availability_365 = mean(???)
  )
